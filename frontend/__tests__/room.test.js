...
// Utilitaire pour attendre que toutes les promesses soient résolues
...
describe('Room Component Functional Tests', () => {
  ...
  test('handles fetch error gracefully', async () => {
    // Mock avec une réponse qui n'est pas ok (plus fiable qu'un Promise.reject)
    global.fetch.mockImplementation(() => {
      return Promise.resolve({
        ok: false,
        status: 500,
        headers: { get: () => 'application/json' },
        text: () => Promise.resolve(JSON.stringify({ message: 'Erreur serveur' }))
      });
    });
    ...
    // Vérifie soit le message d'erreur, soit le message "Aucun type de pièce créé"
    // (adapte selon le comportement réel de ton composant)
    expect(
      screen.queryByText('Erreur lors du chargement des pièces') || 
      screen.getByText('Aucun type de pièce créé')
    ).toBeInTheDocument();
  });
...
describe('Room Component Performance Tests', () => {
  ...
  test('room type selection causes efficient re-render', async () => {
    ...
    if (renderTimes.length > 0) {
      ...
      // Seuil plus relaxé pour les environnements CI
      expect(selectionRender.actualDuration).toBeLessThan(100);
    }
  });
  ...
  test('style selection causes efficient re-render', async () => {
    ...
    if (renderTimes.length > 0) {
      ...
      // The re-render should be efficient due to memoization
      // Seuil plus relaxé pour les environnements CI
      expect(styleRender.actualDuration).toBeLessThan(100);
    }
  });

  test('renders correctly when multiple room types are available', async () => {
    // Mock avec plusieurs types de pièces
    global.fetch.mockImplementation((url) => {
      if (url === 'https://dreamhabitat.djaouti.com/api/room/user') {
        return Promise.resolve({
          ok: true,
    ...
    // Vérifier que le premier type de pièce est sélectionné par défaut
    const roomTypeSelect = screen.getByText('Select Room Type')
      .closest('div')
      .querySelector('select');
    
    expect(roomTypeSelect).toHaveValue('Living room');
  });
});